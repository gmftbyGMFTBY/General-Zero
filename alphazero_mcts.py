...
class TreeNode:
    ...
    def __init__(self, parent, prior_p):
        ...
        self._children = {}      # a map from move to TreeNode, which is a 2-dict
        self._n_visits = 0
        self._Q = 0
        self._u = 0
        self._P = prior_p 
    ...
    def expand(self, action_priors, point):
        ...
        for action, prob in action_priors:
            batch = self._children[point]
            if action not in self._batch:
                batch[action] = TreeNode(self, prob)
    ...
    def select(self, board, c_puct):
        ...
        if batch: return False, None    # this node is the leaf
        return True, max(batch.items(),
...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        node = self._root
        while True:
            is_leaf, action, node = node.select(self._c_puct)
            if is_leaf: break
    ...
    def update_with_move(self, last_move):
        ...
        about the subtree.
        """
        if last_move in self._root._children[state.point]:
            self._root = self._root._children[state.point][last_move]
